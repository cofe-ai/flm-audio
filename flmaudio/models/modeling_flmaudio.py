...
class FLMAudioRotaryEmbedding(nn.Module):
    def __init__(self, config: FLMAudioConfig, device=None):
        super().__init__()
...
class FLMAudioAttention(nn.Module):
    ...
    def __init__(self, config: FLMAudioConfig, layer_idx: int):
        ...
        self.o_proj = nn.Linear(
            config.num_attention_heads * self.head_dim, config.hidden_size, bias=config.attention_bias and not config.disable_att_o_bias
        )
...
class FLMAudioDecoderLayer(nn.Module):
    def __init__(self, config: FLMAudioConfig, layer_idx: int):
        ...
        self.self_attn = FLMAudioAttention(config=config, layer_idx=layer_idx)

        self.mlp = FLMAudioMLP(config)
        self.input_layernorm = FLMAudioRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = FLMAudioRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class MultiModalEmbedding(nn.Module):
    ...
    def forward(
        ...
        for aud_chn_idx in range(self.aud_channel):
            ...
            embeddings += aud_speak_embed + aud_listen_embed
        if self.use_mup:
...
class FLMAudioModel(FLMAudioPreTrainedModel):
    ...
    def __init__(self, config: FLMAudioConfig):
        ...
        self.layers = nn.ModuleList(
            [FLMAudioDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        listen_ids: Optional[torch.LongTensor] = None,
        speak_ids: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids, speak_ids, listen_ids)
...
class FLMAudioForCausalLM(FLMAudioPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.depth_config = DepthGPTConfig(
        ...
        self.aud_output_layers = DepthGPT(self.depth_config)
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if listen_ids is None and speak_ids is None:
            ...
            listen_ids = torch.full(
                (batch_size, seq_len, self.model.config.aud_channel),
                self.model.config.mm_token_info.aud_pad_token_id,
                device=input_ids.device,
                dtype=input_ids.dtype
            )
            speak_ids = torch.full(
                (batch_size, seq_len, self.model.config.aud_channel),
                self.model.config.mm_token_info.aud_pad_token_id,
                device=input_ids.device,
                dtype=input_ids.dtype
    ...
    def forward_audio(self, transformer_output_states, audio_input_ids, step, kv_cache):
        return self.aud_output_layers(transformer_output_states, audio_input_ids, step, kv_cache)
